...
class Player:
    ...
    def move(self):
        ...
        move = self.think(free_places)
        self.env.object_grid[move[0]][move[1]] = self.play
        self.position = str(move[0]) + str(move[1])

        if self.check_win():
            self.won = True
        return self.env.object_grid

    def think(self, free_places):
        random_move = math.floor(random.random() * len(free_places))
        return free_places[random_move]
...
class GeneticAlgorithm():
    ...
    def cross_parents(self):
        new_population = []
        pool = self.gene_pool
        while (len(self.population) - len(self.elites)) != len(new_population):
        ...
        sorted_population = sorted(self.population, key=lambda x: x['fitness'], reverse=True)
        new_population = new_population.extend(sorted_population[:int(len(self.population) * 0.3)])
        ...
        for member in self.population:
            ...
            for index in range(len(self.population)):
                ...
                if not self.play_a_game(member['chromosomes'], self.population[index]['chromosomes'], True):
                    total_games_lost += 1
                if self.play_a_game(member['chromosomes'], self.population[index]['chromosomes'], False):
                    total_games_lost += 1

            member['fitness'] = (total_games-total_games_lost)/total_games
            for i in range(member['fitness']*1000):
...
algo = GeneticAlgorithm(1)
algo.play_a_game('A', 'B', True)
